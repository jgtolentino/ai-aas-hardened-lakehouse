-- =====================================================
-- DESIGN SYSTEM ANALYTICS - COMPLETE DEPLOYMENT
-- =====================================================
-- 
-- INSTRUCTIONS:
-- 1. Copy this entire file
-- 2. Go to: https://app.supabase.com/project/cxzllzyxwpyptfretryc/sql/new
-- 3. Paste and run this SQL
-- 4. Verify deployment with the test queries at the bottom
--
-- =====================================================
...
CREATE TABLE IF NOT EXISTS design_analytics.usage_logs (
...
CREATE INDEX IF NOT EXISTS idx_usage_logs_component ON design_analytics.usage_logs (component_id);
CREATE INDEX IF NOT EXISTS idx_usage_logs_team ON design_analytics.usage_logs (team_id);
CREATE INDEX IF NOT EXISTS idx_usage_logs_created ON design_analytics.usage_logs (created_at);
CREATE INDEX IF NOT EXISTS idx_usage_logs_action ON design_analytics.usage_logs (action);
CREATE INDEX IF NOT EXISTS idx_usage_logs_platform ON design_analytics.usage_logs (platform);

-- Component registry table
CREATE TABLE IF NOT EXISTS design_analytics.components (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
...
CREATE INDEX IF NOT EXISTS idx_components_name ON design_analytics.components (component_name);
CREATE INDEX IF NOT EXISTS idx_components_type ON design_analytics.components (component_type);
CREATE INDEX IF NOT EXISTS idx_components_status ON design_analytics.components (status);
CREATE INDEX IF NOT EXISTS idx_components_library ON design_analytics.components (library_name);

-- Component usage statistics (materialized view for performance)
DROP MATERIALIZED VIEW IF EXISTS design_analytics.component_stats;
...
SELECT 
  ...
  -- Usage metrics
  COALESCE(COUNT(ul.id), 0) AS total_usage,
  COALESCE(COUNT(DISTINCT ul.team_id), 0) AS teams_using,
  COALESCE(COUNT(DISTINCT ul.file_id), 0) AS files_using,
  COALESCE(COUNT(DISTINCT ul.user_id), 0) AS users_using,
  
  -- Action breakdowns
  COALESCE(COUNT(ul.id) FILTER (WHERE ul.action = 'insert'), 0) AS insertions,
  COALESCE(COUNT(ul.id) FILTER (WHERE ul.action = 'detach'), 0) AS detachments,
  COALESCE(COUNT(ul.id) FILTER (WHERE ul.action = 'override'), 0) AS overrides,
  ...
  -- Platform usage
  COALESCE(COUNT(ul.id) FILTER (WHERE ul.platform = 'figma'), 0) AS figma_usage,
  COALESCE(COUNT(ul.id) FILTER (WHERE ul.platform = 'code'), 0) AS code_usage,
  COALESCE(COUNT(ul.id) FILTER (WHERE ul.platform = 'web'), 0) AS web_usage,
...
CREATE INDEX IF NOT EXISTS idx_component_stats_usage ON design_analytics.component_stats (total_usage DESC);
CREATE INDEX IF NOT EXISTS idx_component_stats_detachment ON design_analytics.component_stats (detachment_rate DESC);
CREATE INDEX IF NOT EXISTS idx_component_stats_trend ON design_analytics.component_stats (trend);

-- Team analytics view
CREATE OR REPLACE VIEW design_analytics.team_stats AS
SELECT 
  ul.team_id,
...
CREATE OR REPLACE VIEW design_analytics.library_health AS
SELECT 
  c.library_name,
  c.library_version,
...
CREATE OR REPLACE VIEW design_analytics.insights AS
...
DROP POLICY IF EXISTS "Team usage logs access" ON design_analytics.usage_logs;
DROP POLICY IF EXISTS "Component registry read access" ON design_analytics.components;
DROP POLICY IF EXISTS "Component registry write access" ON design_analytics.components;
...
SELECT 
  ...
  library_name,
  tags
FROM design_analytics.components
ORDER BY component_name;

-- Test 3: Check usage statistics
SELECT 
  component_name,
  total_usage,
  detachment_rate,
...
-- Test 4: View design insights
SELECT 
  component_name,
  insight_type,
  insight_message,
  insight_priority
FROM design_analytics.insights
ORDER BY 
  CASE insight_priority 
    WHEN 'high' THEN 1 
    WHEN 'medium' THEN 2 
...
SELECT 
  ...
  avg_detachment_rate,
  health_score
FROM design_analytics.library_health;

-- =====================================================
-- SUCCESS! 
-- Design System Analytics is now fully deployed
-- =====================================================