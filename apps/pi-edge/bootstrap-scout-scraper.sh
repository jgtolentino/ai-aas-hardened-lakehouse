...
echo "Bootstrapping Scout Scraper Components..."

# 1) Folders
mkdir -p packages/isko-scraper/src
mkdir -p packages/scout-scraper-worker/src
mkdir -p platform/scout/migrations
mkdir -p platform/scout/bruno/requests
mkdir -p supabase/functions/isko-scraper

# 2) SQL migration: queue/cache/dispatcher/recrawl/monitor
cat > platform/scout/migrations/026_scraping_queue.sql <<'SQL'
...
cat > supabase/functions/isko-scraper/index.ts <<'TS'
...
cat > supabase/functions/isko-scraper/supabase.toml <<'TOML'
[functions.isko-scraper]
verify_jwt = false
TOML

# 4) Node worker (polls queue, calls edge function)
cat > packages/scout-scraper-worker/src/worker.ts <<'TS'
...
cat > packages/scout-scraper-worker/package.json <<'JSON'
{
  "name": "@scout/scout-scraper-worker",
  "version": "0.1.0",
  "private": true,
  "type": "module",
  "main": "dist/worker.js",
...
cat > packages/scout-scraper-worker/tsconfig.json <<'JSON'
{
  "compilerOptions": {
    "target": "ES2022",
    "module": "ES2022",
...
# 5) Bruno requests
cat > platform/scout/bruno/requests/30_scrape_seed.bru <<'BRU'
meta {
  name: Seed Jobs from Sources
  type: http
  seq: 30
}

post {
  url: {{PG_REST}}/rpc/seed_jobs_from_sources
  body: json
}
...
cat > platform/scout/bruno/requests/31_get_next_job.bru <<'BRU'
meta {
  name: Get Next Scraping Job
  type: http
  seq: 31
}

post {
  url: {{PG_REST}}/rpc/get_next_job
  body: json
}
...
cat > platform/scout/bruno/requests/33_status.bru <<'BRU'
meta {
  name: Crawl Health Status
  type: http
  seq: 33
}

get {
  url: {{PG_REST}}/v_crawl_health
}
...
# 6) Helper Make targets (append if Makefile exists)
if [ -f Makefile ]; then
  cat >> Makefile <<'MK'
...
echo ""
echo "Next steps:"
echo "1. Deploy edge function: supabase functions deploy isko-scraper"
echo "2. Run migration: make migrate-scout"
echo "3. Seed jobs: make seed-sources"
echo "4. Start worker: make worker"
echo "5. Check status: make scraper-status"