#!/usr/bin/env node
/**
 * W3: Ask Scout hardening - Add cache headers, guardrails, schema validation
...
console.log('🛡️  Hardening Ask Scout with cache and guardrails...');

const askScoutEdgeFunction = `
...
try {
  // Find Edge Functions directory
  const edgeFunctionsPaths = [
    './supabase/functions/ask-scout/index.ts',
    './platform/scout/functions/ask-scout/index.ts',
    './functions/ask-scout/index.ts'
  ];
  
  let targetPath = null;
  for (const p of edgeFunctionsPaths) {
  ...
  if (!targetPath) {
    // Create default path
    targetPath = './supabase/functions/ask-scout/index.ts';
    fs.mkdirSync(path.dirname(targetPath), { recursive: true });
  }
  
  // Write hardened Edge Function
  fs.writeFileSync(targetPath, askScoutEdgeFunction);
  console.log(`✅ Hardened Ask Scout function: ${targetPath}`);
  ...
  const testPrompts = [
    ...
    "Revenue by quarter this year",
    "Brand substitution patterns",
    "Geographic performance heatmap"
  ];
  
  const testFile = path.join(path.dirname(targetPath), 'test-prompts.json');
  fs.writeFileSync(testFile, JSON.stringify(testPrompts, null, 2));
  console.log(`✅ Test prompts created: ${testFile}`);
  
  console.log('🛡️  Ask Scout hardening complete');
  process.exit(0);
  
} catch (error) {
  console.error('❌ Error hardening Ask Scout:', error.message);
  process.exit(1);
}