-- platform/scout/migrations/040_edge_device_schema.sql
-- Adds edge device monitoring and health tracking from Scout v5.2
-- Consolidation of Scout v5.2 + AI-AAS Hardened Lakehouse

-- Create edge device health monitoring table
...
-- Create edge installation checks table
...
CREATE INDEX IF NOT EXISTS idx_edge_health_store_id ON scout.edge_health(store_id);
CREATE INDEX IF NOT EXISTS idx_edge_health_is_online ON scout.edge_health(is_online);
CREATE INDEX IF NOT EXISTS idx_edge_health_last_seen ON scout.edge_health(last_seen);
CREATE INDEX IF NOT EXISTS idx_edge_installation_device_id ON scout.edge_installation_checks(device_id);
...
CREATE OR REPLACE VIEW scout.v_edge_monitoring AS
SELECT 
    eh.device_id,
    eh.device_name,
    eh.store_id,
    ...
    eh.memory_usage,
    eh.disk_usage,
    eh.temperature_celsius,
    CASE 
        WHEN eh.last_seen > NOW() - INTERVAL '5 minutes' THEN 'Active'
        WHEN eh.last_seen > NOW() - INTERVAL '1 hour' THEN 'Idle'
        ELSE 'Offline'
    END as status,
    eic.ready_for_production,
    eic.installation_score
FROM scout.edge_health eh
LEFT JOIN LATERAL (
    SELECT ready_for_production, installation_score
    FROM scout.edge_installation_checks
    WHERE device_id = eh.device_id
    ORDER BY created_at DESC
    LIMIT 1
) eic ON true;

-- Grant appropriate permissions
GRANT SELECT ON scout.edge_health TO authenticated;
GRANT SELECT ON scout.edge_installation_checks TO authenticated;
GRANT SELECT ON scout.v_edge_monitoring TO authenticated;
GRANT EXECUTE ON FUNCTION scout.get_edge_device_status TO authenticated;
GRANT EXECUTE ON FUNCTION scout.run_installation_check TO authenticated;
GRANT EXECUTE ON FUNCTION scout.force_edge_sync TO authenticated;