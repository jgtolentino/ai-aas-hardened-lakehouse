...
-- ===========================================================
create table if not exists scout.gold_contracts (
  id              bigserial primary key,
...
create table if not exists scout.contract_violations (
  id            uuid primary key default gen_random_uuid(),
...
create or replace function scout.verify_gold_contracts()
returns integer
language plpgsql
security definer
set search_path = scout, public
...
-- 1) Agent Feed list
create or replace function scout.rpc_agent_feed_list(
...
create or replace function scout.rpc_monitor_events_list(
  p_limit int default 50,
...
create or replace function deep_research.rpc_sku_summary_list(
...
create or replace function deep_research.rpc_enqueue_sku_job(p_payload jsonb, p_priority int default 100, p_run_after_minutes int default 0)
returns uuid
language plpgsql
security definer
as $$
...
-- ==============================
create or replace function scout.push_feed(
  p_severity text,
  p_source   text,
  p_title    text,
  p_desc     text,
...
create or replace function deep_research.sync_sku_brand()
returns trigger
language plpgsql
as $$
declare v_id uuid;
...
create or replace function masterdata.rpc_brands_list(
...
create or replace function masterdata.rpc_products_list(
...
create or replace function masterdata.generate_catalog(
  p_brand_name text,
  p_category   text,
  p_subcategory text,
...
-- Alaska (Dairy)
create or replace function masterdata.generate_alaska_catalog()
returns integer
language plpgsql
security definer
as $$
...
-- Oishi (Snacks)
create or replace function masterdata.generate_oishi_catalog()
returns integer
language plpgsql
security definer
as $$
...
-- Del Monte (Beverages)
create or replace function masterdata.generate_delmonte_catalog()
returns integer
language plpgsql
security definer
as $$
...
create or replace function masterdata.generate_peerless_catalog()
returns integer
language plpgsql
security definer
as $$
...
-- JTI (Tobacco)
create or replace function masterdata.generate_jti_catalog()
returns integer
language plpgsql
security definer
as $$
...
create or replace function masterdata.generate_client_catalogs()
...
create or replace function masterdata.generate_competitor_catalogs()
returns table(brand text, inserted int)
language plpgsql
security definer
as $$
...
-- ===========================================================
create or replace function masterdata.verify_catalog_import()
returns table(
  check_name text,
  result text
)
